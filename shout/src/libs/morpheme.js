 ...
 * @param {Array<{text: string, p: number, t0: number, reconstructed?: boolean}>} tokens
...
export function buildInterpunctParts(tokens) {
  return tokens.flatMap((token, index) => {
    const reconstructed = token.reconstructed === true;
    const parts = [
      {
        kind: 'token',
        key: `${index}-token`,
        className: `token confidence-${confidenceLevel(token.p)}${reconstructed ? ' reconstructed' : ''}`,
        title: `${reconstructed ? '[Reconstructed] ' : ''}p=${(token.p * 100).toFixed(1)}%  t0=${token.t0 * 10}ms`,
        text: token.text,
      },
    ];
