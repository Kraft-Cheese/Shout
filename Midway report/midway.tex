...
\title{Shout Project Midway Report\\\large Post-ASR Morphological Reconstruction for Polysynthetic Languages}
\author{Project Team}
\date{\today}
...
\midrule
Model artifact mismatch (name/path) & Worker cannot resolve or loads invalid fallback payloads & Enforce naming contract in CI script; add startup checks for expected binaries. \\
Large model footprint on constrained devices & Slow startup or memory pressure & Default to quantized q5 models, cache aggressively, consider tiny/base fallback policy by device class. \\
Over-correction in lexicon reconstruction & Linguistic corruption for confident tokens & Keep uncertainty-trigger gate and token-level selective correction only below threshold. \\
Dataset/domain mismatch & Inflated WER despite improved pipeline & Expand data curation and add targeted held-out test slices per domain/speaker profile. \\
\bottomrule
\end{longtable}
...
\bibitem{shout-root}
Shout Repository, \textit{README.md}, project overview and TODOs.

\bibitem{shout-web}
Shout Web App, \textit{shout/README.md}, Whisper.cpp WASM and model naming conventions.

\bibitem{lora-train}
Shout ASR Model Scripts, \textit{sei\_ncx\_asr\_model/whisper\_lora\_train.py}.

\bibitem{merge-convert}
Shout ASR Model Scripts, \textit{sei\_ncx\_asr\_model/merge\_and\_convert.py}.

\bibitem{b4-reconstruct}
Shout ASR Model Scripts, \textit{sei\_ncx\_asr\_model/reconstruct\_b4.py}.

\bibitem{worker-runtime}
Shout Worker Runtime, \textit{shout/src/workers/whisper.workerThread.js}, \textit{shout/src/workers/worker.js}.
\end{thebibliography}
