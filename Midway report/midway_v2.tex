...
We evaluate transcription quality primarily using \textbf{Word Error Rate (WER)} and \textbf{Character Error Rate (CER)}. Since the project also targets morphologically complex languages, we additionally track morphology-oriented quality using two proxy metrics computed before and after reconstruction:
\begin{itemize}
    \item \textbf{Token F1}, which measures overlap between reference and predicted word tokens;
    \item \textbf{Boundary F1}, which measures agreement in word-boundary positions in the normalized character stream.
\end{itemize}
...
The ASR backbone used in the current experiments is \texttt{openai/whisper-small}. Zero-shot evaluation is performed directly on the target-language test sets. For B2, language-specific LoRA adapters are trained on the corresponding language data. For B4, we apply a lightweight lexicon-constrained reconstruction method that post-processes ASR outputs by replacing out-of-vocabulary words with the closest in-vocabulary candidates under edit distance constraints.

Morphology-oriented evaluation is performed offline using the saved reconstruction outputs. Because no external morphological parser is currently available for \texttt{sei} or \texttt{ncx}, we use token-level F1 and boundary-position F1 as practical proxy measures of structural recovery.
...
The results indicate that lexical reconstruction improves performance for both languages, with substantially larger gains for \texttt{ncx} than for \texttt{sei}. In terms of WER, reconstruction reduces error for both languages, and the morphology-oriented proxy metrics show consistent improvement as well. For \texttt{sei}, the gains are modest but stable, with no observed regressions in the current evaluation. For \texttt{ncx}, the gains are stronger, although a small number of regressions indicate that the reconstructor can occasionally over-correct already valid outputs.

Overall, these findings support the main midway conclusion of the project: a lightweight post-ASR reconstruction stage can improve both transcription quality and structural plausibility in low-resource settings, even before full browser-side validation of all adapted models is complete.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=1.1\linewidth]{figure3.png}
    \caption{Browser runtime inference flow. The app validates runtime assets and model files before transcription, then applies confidence-aware reconstruction only when needed.}
    \label{fig:example}
\end{figure}
\end{document}
